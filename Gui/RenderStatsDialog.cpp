                                                    ...
                                                    const QString& pluginIDFilter)
{
    ...
    if ( useUnixWildcardsCheckbox->isChecked() ) {
        QRegularExpression nameExpr( QRegularExpression::wildcardToRegularExpression(nameFilter), QRegularExpression::CaseInsensitiveOption );
        if ( !nameExpr.isValid() ) {
            return;
        }

        QRegularExpression idExpr( QRegularExpression::wildcardToRegularExpression(pluginIDFilter), QRegularExpression::CaseInsensitiveOption );
        if ( !idExpr.isValid() ) {
            return;
        }
