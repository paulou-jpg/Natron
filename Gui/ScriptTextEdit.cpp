...
PySyntaxHighlighter::highlightBlock(const QString &text)
{
    for (QList<PyHighLightRule>::Iterator it = _imp->rules.begin(); it != _imp->rules.end(); ++it) {
        ...
        while ( match.hasMatch() ) {
            // Format the Nth capture of this match, not the whole match.
            ...
            match = it->pattern.match(text, idx + qMax(length, 1));
        }
    }
                                    ...
                                    const QTextCharFormat &style)
{
    ...
    if (previousBlockState() == inState) {
        ...
        QRegularExpressionMatch first = delimiter.match(text);
        start = first.hasMatch() ? first.capturedStart() : -1;
        // Move past this match
        add = first.hasMatch() ? first.capturedLength() : 0;
    }
    ...
    while (start >= 0) {
        ...
        QRegularExpressionMatch next = delimiter.match(text, start + qMax(length, 1));
