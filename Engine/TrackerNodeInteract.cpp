...
TrackerNodeInteract::refreshSelectedMarkerTexture()
{
    ...
    imageGetterWatcher->setFuture( QtConcurrent::run( [marker, time, roi]() { return marker->getMarkerImage(time, roi); } ) );
}
                                          ...
                                          const TrackMarkerPtr& track)
{
    ...
    if ( !k.roi.isNull() ) {
        ...
        watcher->setFuture( QtConcurrent::run( [track, time, roi = k.roi]() { return track->getMarkerImage(time, roi); } ) );
    }
}
...
TrackerNodeInteract::onTrackImageRenderingFinished()
{
    ...
    if (!selectedMarkerTexture) {
        ...
        selectedMarkerTexture = std::make_shared<Texture>(GL_TEXTURE_2D, GL_LINEAR, GL_NEAREST, GL_CLAMP_TO_EDGE, Texture::eDataTypeByte,
                                              ...
                                              int key)
{
    for (TrackerNodeInteract::TrackKeysMap::iterator it = trackTextures.begin(); it != trackTextures.end(); ++it) {
        if (it->first.lock() == marker) {
            std::map<int, TexturePtr>::iterator found = it->second.find(key);
