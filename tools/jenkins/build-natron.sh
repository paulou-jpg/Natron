...
if [ "$PKGOS" = "Linux" ]; then
    ...
    # fonts.conf is generated into the build tree, so it is not part of the
    # source directory copied above.
    cp "$BUILD_DIR"/Gui/Resources/etc/fonts/fonts.conf "$RES_DIR/etc/fonts/"
fi
...
if [ "$PKGOS" = "Linux" ] || [ "$PKGOS" = "Windows" ]; then
    ...
    # Generated by the CMake build (see Gui/CMakeLists.txt), so it lives in the
    # build tree rather than the source tree as it did under qmake.
    cp "$BUILD_DIR"/Gui/Resources/etc/fonts/fonts.conf "$RES_DIR/etc/fonts/"
